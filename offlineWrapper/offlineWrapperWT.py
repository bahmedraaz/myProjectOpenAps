	...
	for i in range(1,len(data.index)):
		...
		if bg < bgLowerTh:
			...
			# checking if BG is rising
				...
				if delBg> 1.37832346792 and delIob > 0: # row_1 done
					if delInsulinRate < 0:
						data["detection"][i] = 100
						data["unsafe_action_reason"][i] = "row_1"
				#if delBg < 0:
				elif delBg > 1.32621514434 and delIob < 0: # row_2 done
					if delInsulinRate < 0:
						data["detection"][i] = 100
				...
				#if delBg > thBgFall:
				if delBg < abs(-2.70240731915) and delIob > 0: # row_4 done
					if delInsulinRate < 0:
						data["detection"][i] = 100
						data["unsafe_action_reason"][i] = "row_4"
				elif delBg < 0 and delIob < 0: # row_5 done
					if delInsulinRate < 0:
						data["detection"][i] = 100
						data["unsafe_action_reason"][i] = "row_5"
				elif delBg < abs(-0.798450256547) and delIob == 0: # row_6 done
					if delInsulinRate < 0:
						data["detection"][i] = 100
